...
fun SettingsScreen(navController: NavController, onBiometricAuth: () -> Unit) {
    ...
    Scaffold(
        snackbarHost = {
            ...
            AppBarOptBack(
                containerColor = white, text = stringResource(R.string.login_settings), onClick = { navController.popBackStack() })
        ...
        if (passcode==null || useBiometric==null) Box(
            modifier = Modifier
                .fillMaxSize()
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Switch(
                    checked = passcode!!.isNotEmpty(), onCheckedChange = {
                        if (it) {
                        ...
                        checkedThumbColor = white, checkedTrackColor = success06, uncheckedThumbColor = white, uncheckedTrackColor = secondaryGray, uncheckedBorderColor = Color.Transparent
                    )
                )
            }
            if (passcode!!.isNotEmpty()) {
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Spacer(modifier = Modifier.weight(1f))
                    Switch(
                        checked = useBiometric!!, onCheckedChange = {
                            if (it) {
