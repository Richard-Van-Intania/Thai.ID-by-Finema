    ...
    Scaffold(
     topBar = {
         CenterAlignedTopAppBar(
          ...
          actions = {
              IconButton(onClick = { navController.navigate(route = Screen.ProfileEditScreen.route) }) {
                  Icon(imageVector = Icons.Rounded.BorderColor, contentDescription = null, tint = primaryBlack)
              }
          },
     ...
     backgroundColor = white,
        ...
        val birthDate by userCardRepository.birthDate.collectAsState(initial = LocalDateTime.now().toString())
        if (locale == null) Box(modifier = Modifier.fillMaxSize().padding(it), contentAlignment = Alignment.Center) { CircularProgressIndicator() }
        else
         LazyColumn(modifier = Modifier.fillMaxSize().padding(it).padding(horizontal = 16.dp), horizontalAlignment = Alignment.CenterHorizontally) {
             item {
                 ProfileDetailsItem(
                  text = stringResource(R.string.id_number),
                  value =
                   if (idString.isEmpty()) ""
                   else "${idString.substring(0, 1)} ${idString.substring(1, 5)} ${idString.substring(5, 10)} ${idString.substring(10, 12)} ${idString.substring(12)}",
                 )
                      ...
                      text =
                       if (birthDate.isEmpty()) stringResource(R.string.date_format)
                        ...
                        when (locale) {
                            ...
                            TH -> {
                                val localDateTime = LocalDateTime.parse(birthDate)
                                val thaiBuddhistDate = ThaiBuddhistDate.from(localDateTime)
                 ...
                 ProfileDetailsItem(text = stringResource(R.string.middle_name), value = thaiMiddleName)
                 ProfileDetailsItem(text = stringResource(R.string.last_name), value = thaiSurname)
                 ...
                 ProfileDetailsItem(text = stringResource(R.string.middle_name), value = engMiddleName)
                 ProfileDetailsItem(text = stringResource(R.string.last_name), value = engSurname)
...
fun ProfileDetailsItem(text: String, value: String) {
    Row(modifier = Modifier.padding(vertical = 16.dp), verticalAlignment = Alignment.CenterVertically) {
        ...
        Spacer(modifier = Modifier.weight(1f))
        Text(text = if (value.isEmpty()) text else value, color = if (value.isEmpty()) neutral04 else primaryDarkBlue, fontSize = 20.sp, fontWeight = FontWeight.W400)
    }
    HorizontalDivider(thickness = 1.dp, color = neutral02)
}
