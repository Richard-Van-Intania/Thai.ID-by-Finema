...
class UserCardRepository(private val context: Context) {
    ...
    suspend fun updateThaiPrefix(thaiPrefix: String) {
        context.userCardStore.updateData { prefs -> prefs.toBuilder().setThaiPrefix(thaiPrefix).build() }
    }
    ...
    suspend fun updateThaiMiddleName(thaiMiddleName: String) {
        context.userCardStore.updateData { prefs -> prefs.toBuilder().setThaiMiddleName(thaiMiddleName).build() }
    }

    suspend fun updateThaiSurname(thaiSurname: String) {
        context.userCardStore.updateData { prefs -> prefs.toBuilder().setThaiSurname(thaiSurname).build() }
    }
    ...
    suspend fun updateEngMiddleName(engMiddleName: String) {
        context.userCardStore.updateData { prefs -> prefs.toBuilder().setEngMiddleName(engMiddleName).build() }
    }

    suspend fun updateEngSurname(engSurname: String) {
        context.userCardStore.updateData { prefs -> prefs.toBuilder().setEngSurname(engSurname).build() }
    }
